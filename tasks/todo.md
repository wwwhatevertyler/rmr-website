...
- 2026-07-07: Cleaned up contact form Netlify email fields by replacing repeated submitted reason checkboxes with one hidden `reasons` aggregate and one optional `call_link` field. Ran `npm run build`, inspected `dist/rmr-contact.html` for hidden fields and absence of repeated `name="reason"`, and verified the built mobile form POST body in headless Brave produces `reasons=Starting a rollover, Have old 401(k)s`, normalized `phone=+1 817-692-5262`, and `call_link=tel:+18176925262` with no bracketed array output.
